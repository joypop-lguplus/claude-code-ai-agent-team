...
레거시 프로젝트(`sdd-config.yaml`의 `project.type: legacy`)에서는 추가 검증을 수행합니다:

1. **하위 호환성 검증**: 기존 공개 API(함수 시그니처, HTTP 엔드포인트, 데이터 구조)가 보존되었는지 확인합니다.
   - 빌드 전 스펙에 정의된 기존 공개 인터페이스가 변경/삭제되지 않았는지 확인합니다.
   - 기존 API의 동작이 변경된 경우 FAIL로 보고합니다.
2. **기존 테스트 무결성**: 기존 테스트가 모두 여전히 통과하는지 확인합니다.
   - `sdd-config.yaml`의 `test.command`로 전체 테스트를 실행합니다.
   - 기존 테스트가 수정/삭제된 경우 FAIL로 보고합니다.
3. **과잉 수정 검출**: 스펙에 없는 기존 코드 수정이 있는지 확인합니다.
   - Git diff를 분석하여 변경된 파일/라인을 파악합니다.
   - 체크리스트 항목과 무관한 코드 변경이 있으면 FAIL로 보고합니다.
4. **마이그레이션 검증**: `04-data-migration.md`(레거시 스펙)의 마이그레이션 계획이 구현되었는지 확인합니다.
