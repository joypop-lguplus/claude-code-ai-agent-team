...
신규 프로젝트와 **동일한 파일명**으로 다음 문서를 생성합니다. 각 문서는 현행 시스템을 기준으로 문서화하되, `{{#if LEGACY}}` 블록에 변경 태그와 영향 분석 섹션을 추가합니다:
1. `02-architecture.md` — 현행 아키텍처 + [레거시 추가] 변경 영향 분석 섹션 (영향 모듈, 위험 평가, 마이그레이션 계획)
2. `03-api-spec.md` — 현행 API 전체 문서화 + [레거시 추가] 변경 태그 (`UNCHANGED`/`MODIFY`/`NEW`/`DEPRECATE`)
3. `04-data-model.md` — 현행 데이터 모델 + [레거시 추가] 마이그레이션 대상 태그
4. `05-component-breakdown.md` — 현행 컴포넌트 + [레거시 추가] 변경 대상 태그
5. `06-spec-checklist.md` — 검증 가능한 모든 항목을 포함한 구현 체크리스트
