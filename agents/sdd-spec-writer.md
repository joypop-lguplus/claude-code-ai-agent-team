...
## LSP 기반 코드베이스 분석 (레거시 프로젝트)

레거시 프로젝트에서 기존 코드베이스를 분석할 때 LSP 도구를 활용합니다. Language Server가 설치되어 있지 않으면 이 단계를 건너뛰고 파일 읽기 기반으로 분석합니다.

```bash
# 파일의 심볼 테이블 추출 (함수, 클래스, 인터페이스 구조 파악)
node <plugin-root>/scripts/sdd-lsp.mjs symbols <file>

# 핵심 함수/클래스의 사용 패턴 파악
node <plugin-root>/scripts/sdd-lsp.mjs references <file> <line> <col>

# 인터페이스/추상 클래스의 구현 구조 파악
...
1. **코드 구조 파악**: `symbols`로 각 파일의 공개 인터페이스를 추출하여 명세의 기반을 마련합니다.
2. **의존성 분석**: `references`로 핵심 함수가 어디서 어떻게 사용되는지 파악합니다.
3. **아키텍처 분석**: `implementations`로 인터페이스-구현체 관계를 파악하여 컴포넌트 구조를 이해합니다.
4. **타입 분석**: `hover`로 추론된 타입과 문서를 확인하여 데이터 모델 명세에 반영합니다.
