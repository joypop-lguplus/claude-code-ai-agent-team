...
- **`/claude-sdd:lsp` 스킬**: LSP 기반 의미 분석 — 10개 서브커맨드 (diagnostics, definition, references, hover, symbols, workspace-symbols, implementations, incoming-calls, outgoing-calls, status)
...
- **`/claude-sdd:lint` 스킬**: 4개 서브커맨드를 통한 코드 분석 (diagnostics, search, symbols, format)
...
- **품질 게이트 자동 진단**: `/claude-sdd:review` 2.5단계에 에러 0건 기준 적용

### 변경

- **`/claude-sdd:review`**: sdd-code-analyzer를 통한 자동 진단 2.5단계 추가
- **`/claude-sdd:build`**: 워크 패키지 완료 전 린트/포맷 검사 3.5단계 추가
...
- **9개 스킬**: `/claude-sdd:auto`, `/claude-sdd:init`, `/claude-sdd:intake`, `/claude-sdd:spec`, `/claude-sdd:plan`, `/claude-sdd:build`, `/claude-sdd:review`, `/claude-sdd:integrate`, `/claude-sdd:status`
