...
**핵심 목표**: `/sdd-review` 품질 게이트와 `/sdd-build` 구현 단계에서 자동화된 코드 품질 검증을 추가한다.
...
| 파일 | 역할 |
|------|------|
| `skills/sdd-lint/SKILL.md` | `/sdd-lint` 스킬 — 진단, 구조 검색, 포매팅 |
...
| 파일 | 변경 내용 |
|------|-----------|
| `README.md` | `/sdd-lint` 스킬, 코드 분석 에이전트 문서화 |
| `CHANGELOG.md` | v0.2.0 변경 사항 추가 |
| `docs/architecture.md` | 코드 분석 레이어 추가 |
| `docs/usage-guide.md` | `/sdd-lint` 사용 예시 추가 |

---

## 상세 설계

### 1. `/sdd-lint` 스킬 (`skills/sdd-lint/SKILL.md`)

oh-my-opencode의 6개 LSP 도구를 4개 서브커맨드로 매핑:

```
/sdd-lint diagnostics [path]      # 프로젝트 에러/경고 수집 (tsc, ruff, cargo check 등)
/sdd-lint search <pattern> [path] # ast-grep 구조적 코드 검색
/sdd-lint symbols [path]          # 함수/클래스/export 심볼 추출
/sdd-lint format [path]           # 코드 포매팅 (prettier, biome, rustfmt 등)
```
...
SDD 라이프사이클과의 통합:
- `/sdd-spec` 단계: 레거시 프로젝트에서 기존 코드 구조 분석
- `/sdd-build` 단계: 구현 완료 시 자동 린트/포맷
- `/sdd-review` 단계: 진단 결과를 품질 게이트에 포함
...
**`/sdd-review` 수정**: Step 2.5로 "자동 진단 실행" 추가
```
...
**`/sdd-build` 수정**: 팀 멤버 완료 보고 전 린트/포맷 권장 추가
```
