...
인터뷰 (7개 섹션, 섹션 7: 프로젝트 규칙)
  → sdd-init → Phase 2.5: 규칙 파일 생성 → sdd-intake → sdd-spec → ①체크포인트 → sdd-plan → sdd-assign → sdd-build → ②체크포인트 → sdd-review → sdd-integrate
```

갓모드는 `spec_depth: thorough`로 DDL 수준의 상세 스펙을 생성합니다. 섹션 7에서 프로젝트 규칙(아키텍처, 코딩 컨벤션, API 설계, 에러 처리, 테스트, 보안)을 인터뷰하고, Phase 2.5에서 기술 스택에 맞는 프리셋을 매칭하여 규칙 파일을 자동 생성합니다. Phase 3 파이프라인에서는 주요 단계 완료 후 체크포인트(①sdd-spec 후 산출물 검증, ②첫 WP 빌드 후 통과율 검증)를 통해 문제를 조기에 감지하고 불필요한 단계 실행을 방지합니다.
...
레거시 모드의 빌드 단계는 **분석 전용**입니다. 코드 변경 없이 기존 코드와 스펙을 대조 분석만 수행합니다. 에이전트에는 레거시 분석 모드에 필요한 섹션만 선별 주입됩니다:
...
최대 3회까지 재작업이 시도됩니다 (팀 모드: 리더가 멤버에게 지시, 솔로 모드: 직접 재구현). 팀 모드에서는 WP 완료 즉시 해당 체크리스트를 검증하는 **점진적 검증**으로, 다른 멤버의 완료를 기다리지 않고 즉시 재작업을 지시합니다. 3회 실패 시 사용자에게 에스컬레이션됩니다. 직접 개입하여 코드를 수정한 뒤 `/claude-sdd:sdd-build`를 다시 실행할 수 있습니다.
