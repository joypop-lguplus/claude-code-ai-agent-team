...
4. **에이전트 모델 = 역할별 분리**: 사고가 필요한 작업(구현, 테스트, 리뷰, 변경 분석)에는 Sonnet, 도구 실행 위주 작업(요구사항 수집, 코드 분석, 린트)에는 Haiku를 사용합니다. `sdd-config.yaml`의 `teams.model`과 `teams.lightweight_model`로 설정합니다.
5. **토큰 최적화**: 에이전트 프롬프트에 모드별 필요 섹션만 선별 주입하고, 체크리스트는 WP 배정 항목만 인라인 포함하며, 템플릿의 조건부 블록을 설정에 따라 사전 제거합니다.
...
│   ├── requirements-analyst  # 소스 파싱 (Haiku)
│   ├── spec-writer           # 스펙 생성
...
**템플릿 연동**: `sdd-leader.md.tmpl`과 `sdd-member.md.tmpl`에 에이전트 규칙 참조 지침이 포함되어, 중복 없이 에이전트 정의의 LSP/린트 섹션을 따릅니다. `sdd-assign`에서 조건부 블록(`{{#if}}`)을 설정에 따라 사전 처리하여 불필요한 모드 지시를 제거합니다.
...
3. **점진적 검증**: 멤버 완료 보고 즉시 해당 WP의 배정 항목만 Grep으로 확인 (전체 체크리스트 재읽기 방지)
4. 미완료 항목에 구체적이고 실행 가능한 피드백을 **즉시** 제공 (다른 멤버 완료 대기 없이)
5. 3회 재작업 사이클 후 사용자에게 에스컬레이션
6. 전체 WP 완료 후 최종 교차 검증 1회 수행
7. 할당된 항목이 100% `[x]`일 때만 다음 단계로 진행
