...
**레거시 프로젝트** 산출물 (신규와 동일한 파일명, 레거시 전용 섹션 추가):
- 아키텍처 문서 + 변경 영향 분석 섹션
- API 스펙 + 변경 태그 (UNCHANGED/MODIFY/NEW/DEPRECATE)
- 데이터 모델 + 마이그레이션 대상 태그
- 컴포넌트 분해 + 변경 대상 태그
- 스펙 준수 체크리스트
...
멤버 CLAUDE.md 생성 시, `sdd-config.yaml` 설정에 따라 불필요한 조건부 블록(`{{#if TDD_MODE}}`, `{{#if LEGACY_MODE}}` 등)이 자동 제거되어 컨텍스트가 최적화됩니다. 리더 CLAUDE.md에도 동일한 조건부 블록 처리가 적용됩니다.
...
팀 멤버가 병렬로 구현               WP를 순차적으로 직접 구현
  |                                  |
WP별 즉시 체크리스트 검증           체크리스트 항목 검증
  |-- 미완료? --> 즉시 재작업          |-- 미완료? --> 직접 재구현
  |-- 완료? --> 다음 단계              |-- 완료? --> 다음 WP
  |-- 3회 실패? --> 에스컬레이션       |-- 3회 실패? --> 에스컬레이션
전체 WP 완료 후 최종 교차 검증
```

**토큰 최적화**: 각 멤버에게 WP에 배정된 체크리스트 항목만 선별 전달하고, 에이전트 정의도 현재 모드에 필요한 섹션만 주입하여 컨텍스트 비용을 절감합니다. 팀 모드에서는 WP 완료 즉시 해당 항목을 검증하는 점진적 검증으로 재작업 지연을 최소화합니다.
...
- `/claude-sdd:sdd-build` 단계: 워크 패키지 완료 전 진단 + 포맷 실행 (경량 모델 사용)
...
2. **영향 분석 + 스펙 업데이트**: `sdd-change-analyst`가 기존 스펙 대비 파급 효과 분석 → 기존 02~05 문서에 변경 사이클 섹션 추가
...
1. **갓모드 인터뷰** (`/claude-sdd:sdd-godmode`): 섹션 7에서 아키텍처, 코딩 컨벤션, API 설계, 에러 처리, 테스트, 보안 규칙을 질문합니다. 기술 스택에 맞는 프리셋이 자동 적용됩니다. Phase 3 파이프라인에서는 주요 단계(sdd-spec 완료, 첫 WP 빌드 완료) 후 체크포인트를 통해 문제를 조기에 감지합니다.

2. **일반 모드 수집** (`/claude-sdd:sdd-intake`): 초회 실행 시 `00-project-rules.md`가 없으면 갓모드와 동일한 규칙 수집/생성을 수행합니다. 신규 프로젝트는 7항목 인터뷰, 레거시 프로젝트는 코드 자동 분석 → 사용자 확인 방식입니다.

3. **레거시 자동 감지**: 레거시 프로젝트에서는 기존 코드의 패턴(패키지 구조, import 패턴, API 패턴, 테스트 패턴)을 분석하여 규칙을 자동 감지합니다.
...
- **파이프라인 체크포인트**: 갓모드 Phase 3에서 주요 단계 완료 후 산출물 무결성을 검증합니다. 문제 발견 시 조기 중단하여 불필요한 토큰 소비를 방지합니다.
