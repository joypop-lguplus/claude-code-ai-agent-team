...
# /claude-sdd:auto — SDD 라이프사이클 오케스트레이터
...
```
/claude-sdd:auto                   # 단계 자동 감지 후 진행
/claude-sdd:auto help              # 모든 SDD 명령어 표시
/claude-sdd:auto reset             # SDD 상태 초기화 (확인 필요)
```

## 동작

### 자동 감지 모드 (`/claude-sdd:auto`)

프로젝트 상태를 읽고 다음 작업으로 라우팅합니다:

1. **`sdd-config.yaml`이 없음** → `/claude-sdd:init` 실행
2. **`01-requirements.md`가 없음** → `/claude-sdd:intake` 실행
3. **`02-*.md`부터 `06-*.md`가 없음** → `/claude-sdd:spec` 실행
4. **`07-task-plan.md`가 없음** → `/claude-sdd:plan` 실행
5. **`07-task-plan.md` 존재, 체크리스트 미완료** → `/claude-sdd:build` 실행
6. **체크리스트 완료, `08-review-report.md` 없음** → `/claude-sdd:review` 실행
7. **리뷰 통과** → `/claude-sdd:integrate` 실행
8. **모두 완료** → 완료 요약 표시
...
### 초기화 모드 (`/claude-sdd:auto reset`)

확인을 요청한 후:
1. `docs/specs/` 내 모든 파일 삭제
2. `CLAUDE.md`에서 SDD 규칙 제거
3. 출력: `SDD 상태가 초기화되었습니다. /claude-sdd:init을 실행하여 다시 시작하세요.`
