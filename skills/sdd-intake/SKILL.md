...
2. **프로젝트 유형 감지**: `project.type` 값을 확인합니다:
   - `new` → **신규 프로젝트 모드** (기존 동작)
   - `legacy` → **레거시 프로젝트 모드** (인터뷰 없이 기존 코드 자동 분석)
3. **도메인 모드 감지**: `domains` 키가 존재하는지 확인합니다:
...
> **참고**: `type: legacy` 프로젝트에서는 Confluence/Jira/File 소스와 함께 기존 코드베이스 자동 분석도 병행합니다. 외부 소스에서 수집된 요구사항에 코드 분석 결과가 추가됩니다.
...
인터뷰를 수행하지 않습니다. 기존 코드베이스를 자동 분석하여 `01-requirements.md`를 생성합니다:

1. **프로젝트 구조 탐색**: Glob으로 소스 디렉토리, 설정 파일, 테스트 디렉토리를 파악
2. **기술 스택 감지**: `scripts/sdd-detect-tools.sh`를 실행하여 언어, 프레임워크, 린터/포매터 자동 감지
3. **진입점 및 주요 모듈 파악**: `package.json`, `go.mod`, `pom.xml` 등에서 프로젝트 메타 정보 추출
4. **테스트 현황 파악**: 테스트 파일 수, `test.command`가 있으면 실행하여 커버리지 확인
5. **기존 API/공개 인터페이스 파악**: 라우트, 컨트롤러, export를 Grep/Glob으로 탐색
6. **`00-project-context.md` 참조** (godmode 실행인 경우): 심층 인터뷰에서 수집된 기술 스택, NFR, 코드 규칙 정보를 반영

분석 결과를 사용자에게 요약 표시 후 `01-requirements.md`를 생성합니다.
출력 형식은 신규 프로젝트와 동일 (`FR-NNN`, `NFR-NNN`).
...
요구사항 수집이 완료된 후, `docs/specs/00-project-rules.md`가 **존재하지 않는 경우에만** 프로젝트 규칙을 수집하고 생성합니다. 갓모드(`sdd-godmode`)에서 이미 생성되었으면 이 단계를 건너뜁니다.

#### 신규 프로젝트의 경우 (type: new)

갓모드 섹션 7과 동일한 7항목을 질문합니다:

> **규모 적응**: FR이 10개 미만인 MVP/토이 프로젝트에서는 간소화합니다:
...
갓모드 섹션 7L과 동일한 코드 자동 분석을 수행합니다:

1. **자동 분석**: 패키지 구조에서 아키텍처 패턴, import 패턴에서 네이밍 컨벤션, 컨트롤러/라우터에서 API URL 패턴, 에러 핸들러에서 에러 처리 방식, 테스트 디렉토리에서 테스트 패턴을 감지합니다.
2. **감지 결과 제시**: 추출된 규칙 요약을 사용자에게 표시합니다.
3. **사용자 확인**: `AskUserQuestion`으로 "감지된 규칙이 맞나요?" 한 번만 질문합니다.
...
2. **규칙 파일 생성**: `templates/rules/` 템플릿을 사용하여 9개 파일을 생성합니다:
   - `docs/specs/00-project-rules.md` (인덱스)
   - `docs/specs/rules/architecture.md` ~ `performance.md` (8개 카테고리)
3. **sdd-config.yaml 업데이트**: `rules` 섹션을 활성화합니다.
...
**레거시 프로젝트의 경우** 추가 출력:
```
요구사항 문서가 생성되었습니다: docs/specs/[domains/<id>/]01-requirements.md (자동 분석)
  - X개의 기능 요구사항 (기존 코드에서 추출)
  - Y개의 비기능 요구사항
  - Z개의 API/공개 인터페이스 식별
...
**규칙 생성 시** 추가 출력 (초회, `00-project-rules.md` 미존재 시):
```
프로젝트 규칙이 생성되었습니다: docs/specs/00-project-rules.md
  - 프리셋: [프리셋명]
  - 카테고리: 8개
  - 규칙 수: [N]개
  - 적용 모드: [strict/advisory]
```
