...
export async function runInstaller(options = {}) {
  ...
  const lspServers = [
    { name: 'typescript-language-server', label: 'TypeScript LSP', brew: 'typescript-language-server', npm: 'typescript-language-server typescript', pip: null, hint: 'npm i -g typescript-language-server typescript' },
    { name: 'pyright-langserver', label: 'Python LSP (Pyright)', brew: 'pyright', npm: 'pyright', pip: 'pyright', hint: 'npm i -g pyright' },
    { name: 'gopls', label: 'Go LSP (gopls)', brew: 'gopls', npm: null, pip: null, goInstall: 'golang.org/x/tools/gopls@latest', hint: 'go install golang.org/x/tools/gopls@latest' },
    { name: 'rust-analyzer', label: 'Rust LSP', brew: 'rust-analyzer', npm: null, pip: null, rustup: 'rust-analyzer', hint: 'rustup component add rust-analyzer' },
    { name: 'clangd', label: 'C/C++ LSP (clangd)', brew: 'llvm', npm: null, pip: null, hint: 'brew install llvm (macOS) 또는 OS 패키지 매니저' },
    { name: 'jdtls', label: 'Java LSP (jdtls)', brew: 'jdtls', npm: null, pip: null, hint: 'brew install jdtls (macOS) 또는 https://projects.eclipse.org/projects/eclipse.jdt.ls' },
    { name: 'kotlin-language-server', label: 'Kotlin LSP', brew: 'kotlin-language-server', npm: null, pip: null, hint: 'brew install kotlin-language-server (macOS) 또는 https://github.com/fwcd/kotlin-language-server' },
  ];
  ...
  for (const server of lspServers) {
    ...
    // 사용 가능한 설치 방법 중 우선순위가 높은 것을 자동 선택
    let method = null;
    if (server.npm && commandExists('npm')) {
      method = { type: 'npm', cmd: server.npm };
    } else if (server.pip && commandExists('pip')) {
      method = { type: 'pip', cmd: server.pip };
    } else if (server.goInstall && commandExists('go')) {
      method = { type: 'go', cmd: server.goInstall };
    } else if (server.rustup && commandExists('rustup')) {
      method = { type: 'rustup', cmd: server.rustup };
    } else if (server.brew && isMac && commandExists('brew')) {
      method = { type: 'brew', cmd: server.brew };
    }

    if (!method) {
      status(server.label, false, '설치 불가');
    ...
    let result;
    if (method.type === 'npm') {
      console.log(colors.dim(`  npm i -g ${method.cmd} 실행 중...`));
      result = spawnSync('npm', ['i', '-g', ...method.cmd.split(' ')], { stdio: 'inherit' });
    } else if (method.type === 'pip') {
      console.log(colors.dim(`  pip install ${method.cmd} 실행 중...`));
      result = spawnSync('pip', ['install', method.cmd], { stdio: 'inherit' });
    } else if (method.type === 'go') {
      console.log(colors.dim(`  go install ${method.cmd} 실행 중...`));
      result = spawnSync('go', ['install', method.cmd], { stdio: 'inherit' });
    } else if (method.type === 'rustup') {
      console.log(colors.dim(`  rustup component add ${method.cmd} 실행 중...`));
      result = spawnSync('rustup', ['component', 'add', method.cmd], { stdio: 'inherit' });
    } else if (method.type === 'brew') {
      console.log(colors.dim(`  brew install ${method.cmd} 실행 중...`));
      result = spawnSync('brew', ['install', method.cmd], { stdio: 'inherit' });
    }
