...
async function configureDiagramTools() {
  ...
  if (!await confirm(`  ${sym.arr} 다이어그램 생성 도구를 설치하시겠습니까?`)) {
    console.log(colors.dim('  건너뜀. 다이어그램 PNG 생성 기능을 사용할 수 없습니다.'));
    return;
  }
  ...
  if (!mmdc) {
    if (commandExists('npm')) {
      ...
      const r = spawnSync('npm', ['i', '-g', '@mermaid-js/mermaid-cli'], { stdio: 'inherit', timeout: 120000 });
      if (r.status === 0) {
  ...
  if (hasMermaidMcp) {
    ...
    if (await confirm(`  ${sym.arr} claude-mermaid MCP (브라우저 다이어그램 프리뷰)를 설정하시겠습니까?`)) {
      config.mcpServers = config.mcpServers || {};
      ...
      console.log(colors.dim('  건너뜀. 수동 설정: npx claude-mermaid'));
    }
  }
}
