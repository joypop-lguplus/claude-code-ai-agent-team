...
const USER_COMMANDS_DIR = join(home, '.claude', 'commands');

// SDD 커맨드 파일 목록 (commands/ 디렉토리에서 복사)
const SDD_COMMAND_PREFIX = 'sdd';

/**
 * commands/ 디렉토리의 커맨드 파일을 ~/.claude/commands/에 설치합니다.
 * @returns {number} 설치된 커맨드 수
 */
function installCommands() {
  ...
  for (const file of files) {
    ...
    const dest = join(USER_COMMANDS_DIR, file);
    try {
...
/**
 * ~/.claude/commands/에서 SDD 커맨드 파일을 제거합니다.
 * @returns {number} 제거된 커맨드 수
 */
function uninstallCommands() {
  if (!existsSync(USER_COMMANDS_DIR)) return 0;

  const files = readdirSync(USER_COMMANDS_DIR)
    .filter((f) => f.startsWith(SDD_COMMAND_PREFIX) && f.endsWith('.md'));
  let removed = 0;

  for (const file of files) {
    try {
      unlinkSync(join(USER_COMMANDS_DIR, file));
      removed++;
    } catch { /* ignore */ }
  }
...
export async function runInstaller(options = {}) {
  ...
  // -- Step 5: Plugin & Commands Registration --
  section(colors.bold('[5/5] 플러그인 등록 및 커맨드 설치'));
  ...
  // 슬래시 명령어 설치 (~/.claude/commands/)
  const cmdCount = installCommands();
  if (cmdCount > 0) {
    status('슬래시 명령어', true, `${cmdCount}개 설치됨 → ${USER_COMMANDS_DIR}`);
  } else {
    status('슬래시 명령어', false, '설치할 커맨드 파일이 없습니다');
  }
  ...
  console.log(`  1. ${colors.bold('claude')} 을 실행하세요`);
...
export async function runUninstaller() {
  ...
  // 2. 슬래시 명령어 제거 (~/.claude/commands/)
  section('슬래시 명령어 제거');
  const cmdRemoved = uninstallCommands();
  if (cmdRemoved > 0) {
    status('슬래시 명령어', true, `${cmdRemoved}개 제거 완료`);
    removed = true;
  } else {
    console.log(colors.dim('  설치된 SDD 커맨드가 없습니다.'));
  }
