...
export async function runUninstaller() {
  ...
  const diagTools = [];
  if (installed.diagrams.mmdc) diagTools.push('mmdc (Mermaid CLI)');
  ...
  if (diagTools.length > 0) {
    ...
    // mmdc (Mermaid CLI) 제거
    if (installed.diagrams.mmdc && commandExists('npm')) {
      const r = spawnSync('npm', ['uninstall', '-g', '@mermaid-js/mermaid-cli'], { stdio: 'pipe', timeout: 60000 });
      if (r.status === 0) {
        status('mmdc (Mermaid CLI)', true, '제거 완료');
        removed = true;
      } else {
        status('mmdc (Mermaid CLI)', false, '제거 실패 — npm uninstall -g @mermaid-js/mermaid-cli');
      }
    }

    // atlassian-python-api 제거
    if (installed.diagrams.atlassianApi) {
      const r = spawnSync('pip3', ['uninstall', '-y', '--break-system-packages', 'atlassian-python-api'], { stdio: 'pipe', timeout: 60000 });
      if (r.status === 0) {
        status('atlassian-python-api', true, '제거 완료');
        removed = true;
      } else {
        status('atlassian-python-api', false, '제거 실패');
      }
    }

    // claude-mermaid MCP 제거
    const claudeJson = join(home, '.claude.json');
    if (existsSync(claudeJson)) {
      try {
        const config = JSON.parse(readFileSync(claudeJson, 'utf8'));
        if (config.mcpServers?.['claude-mermaid']) {
          delete config.mcpServers['claude-mermaid'];
          writeFileSync(claudeJson, JSON.stringify(config, null, 2) + '\n');
          status('claude-mermaid MCP', true, '제거 완료');
          removed = true;
        }
      } catch { /* ignore */ }
    }
  ...
  if (extraTools.length > 0) {
    ...
    if (installed.tools.uv) {
      let uvRemoved = false;
      // standalone installer (~/.local/bin/uv, ~/.local/bin/uvx)
      const uvBin = join(home, '.local', 'bin', 'uv');
      const uvxBin = join(home, '.local', 'bin', 'uvx');
      if (existsSync(uvBin)) {
        try { unlinkSync(uvBin); uvRemoved = true; } catch { /* ignore */ }
        try { if (existsSync(uvxBin)) unlinkSync(uvxBin); } catch { /* ignore */ }
      }
      // brew로 설치된 경우
      if (!uvRemoved && isMac && commandExists('brew')) {
        const r = spawnSync('brew', ['uninstall', 'uv'], { stdio: 'pipe', timeout: 60000 });
        uvRemoved = r.status === 0;
      }
      status('uv', uvRemoved, uvRemoved ? '제거 완료' : '제거 실패');
      if (uvRemoved) removed = true;
    }
