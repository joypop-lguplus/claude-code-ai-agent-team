...
import {
  colors, sym, status, section, ensureBrewNodeLinked,
} from './utils.mjs';
...
export function runUninstaller() {
  ...
  let hasMermaidMcp = false;
  {
    const claudeJson = join(home, '.claude.json');
    if (existsSync(claudeJson)) {
      try {
        const config = JSON.parse(readFileSync(claudeJson, 'utf8'));
        hasMermaidMcp = !!config.mcpServers?.['claude-mermaid'];
      } catch { /* ignore */ }
    }
  }
  if (hasMermaidMcp) targets.push('claude-mermaid MCP');
