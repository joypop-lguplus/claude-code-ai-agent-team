 ...
 */
/**
 * brew 작업 후 node symlink가 깨졌으면 복원합니다. (macOS 전용)
 * @returns {boolean} node가 정상 동작하면 true
 */
export function ensureBrewNodeLinked() {
  if (!isMac) return true;
  if (commandExists('node')) return true;
  try {
    const r = spawnSync('brew', ['link', 'node', '--overwrite'], { stdio: 'pipe', timeout: 30000 });
    if (r.status === 0) {
      status('Node.js', true, 'brew symlink 복원 완료');
      return true;
    }
  } catch { /* ignore */ }
  return false;
