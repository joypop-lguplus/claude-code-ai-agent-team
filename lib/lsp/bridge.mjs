...
export class LspBridge {
  ...
  async connect(language, rootPath) {
    ...
    const finalArgs = [...config.args, ...(config.dynamicArgs ? config.dynamicArgs(this.#rootPath) : [])];
    this.#client.start(config.command, finalArgs, { cwd: this.#rootPath });

    // initialize 핸드셰이크
    const initTimeout = config.initTimeout || 30_000;
