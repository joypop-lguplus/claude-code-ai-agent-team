...
export function printResults(results) {
  const categories = [
    { key: 'core',    title: '[1/4] Core Tools' },
    { key: 'claude',  title: '[2/4] Claude Code & Agent Teams' },
    { key: 'github',  title: '[3/4] GitHub CLI' },
    { key: 'mcp',     title: '[4/4] MCP Servers (Optional)' },
  ];
  ...
  const required = results.filter(r => r.category !== 'mcp');
  const missing = required.filter(r => !r.ok);
  const optional = results.filter(r => r.category === 'mcp' && !r.ok);
