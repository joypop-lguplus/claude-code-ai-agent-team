...
export function checkAll() {
  ...
  const lspServers = [
    ...
    { name: 'jdtls', label: 'Java LSP (jdtls)', install: 'brew install jdtls 또는 eclipse.org/jdtls' },
    { name: 'kotlin-language-server', label: 'Kotlin LSP', install: 'brew install kotlin-language-server 또는 github.com/fwcd/kotlin-language-server' },
  ];
  ...
  // -- MCP Servers (optional, file-based check) --
  // claude mcp list는 행(hang)이 발생할 수 있으므로 설정 파일을 직접 확인합니다.
  let mcpConfig = '';
  const mcpPaths = [
  ...
  for (const p of mcpPaths) {
    if (existsSync(p)) {
      try { mcpConfig += readFileSync(p, 'utf8'); } catch { /* ignore */ }
    }
  }

  const confluenceOk = mcpConfig.includes('confluence') || mcpConfig.includes('atlassian');
  results.push({
    name: 'Confluence MCP',
    ok: confluenceOk,
  ...
  const jiraOk = mcpConfig.includes('jira') || mcpConfig.includes('atlassian');
  results.push({
    name: 'Jira MCP',
    ok: jiraOk,
